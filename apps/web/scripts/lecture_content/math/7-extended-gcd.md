...
回傳的 `x` 可能為負，取模後要 `(x % m + m) % m` 校正；只有在 `gcd(a, m) == 1` 時逆元才存在，否則無解；解 `ax + by = c` 時忘記先檢查 `g` 整除 `c`，或忘記把特解乘上 `c / g`；中間乘積溢位需用 `long long` 甚至 `__int128`。
...
`x = y₁`、`y = x₁ - ⌊a/b⌋·y₁`；出口取 `(1, 0)`；逆元存在的條件是互質；結果記得校正成非負。
