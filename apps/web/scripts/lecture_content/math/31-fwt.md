...
陣列長度不是 2 的冪（須先補零至 $2^m$）；逆變換忘記除以 `n`；在模意義下用整數除法而非乘以 `n` 的逆元；中間值溢位需 `long long`；把 XOR 的蝴蝶運算誤用在 AND/OR 卷積上（三者的變換不同）。
...
正變換、逐點乘、逆變換；XOR 的蝴蝶是 $(x+y, x-y)$；逆變換要除以 `n`；長度必須是 2 的冪。
