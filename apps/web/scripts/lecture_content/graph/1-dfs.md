...
在無向圖判環時把「走回父節點」誤判成環（需另外傳入父節點或以邊編號排除）；用 `color[y] == 2` 當成環的條件（那只是重複到達，不是環）；忘記圖可能不連通而只從節點 `0` 出發；鏈狀圖遞迴過深造成堆疊溢位，此時改寫成手動堆疊的迭代版本。
...
每點展開一次、每邊檢查一次；前序往下傳、後序往上收；有向圖判環看的是「在路徑上」而不是「已訪問」。
