...
兩份統計量共用同一個表（必須各自獨立，否則收縮會互相干擾）；`left1` 與 `left2` 的角色寫反，導致差值為負；`k` 為 0 時「至多 `-1`」永遠不成立，`left2` 會推進到 `right + 1`，需確認題意或特判；次數歸零時忘記從表中刪除，使 `size()` 失真。
...
共用右端點、兩個左端點；`left1 <= left2`；累加 `left2 - left1`；兩份統計量必須各自獨立。
