const studyPlanCourseMaterials: Record<string, string> = {
  technical_interview: [
    ...
    "**題單情境對照**",
    "這份題單把常見系統面試情境（服務拓撲、矩陣批次處理、依賴 DAG、排程決策、時間窗並發、資料流摘要、日誌解析、C++ 資源邊界、快取索引、API 設計）對應到演算法 pattern。練習時建議把每題先歸類成一個已知 pattern，再補上面試需要的溝通：狀態定義、正確性、不變式、複雜度、記憶體上限與邊界條件。",
  ].join("\n\n"),
  ...
  binary_search: [
    ...
    "**模式與模板速查**",
    "**Pattern 1：lower_bound / upper_bound**\n\n訊號：排序陣列中找第一個 `>= x`、第一個 `> x`、最後一個 `< x`。不變式是答案始終在 `[lo, hi)`。「找最後一個合法位置」不必另寫一套，轉成「找第一個不合法的位置再減一」即可。",
  ...
  data_structure: [
    ...
    "**Pattern 5：Ordered set**\n\n訊號：要在動態集合裡查前驅、後繼或區間端點，而不只是最大最小。C++ 用 `set` / `multiset` 搭配 `lower_bound`；同值會重複出現時記得用 `multiset`，刪除也要用迭代器而非值。",
  ].join("\n\n"),
  ...
  graph: [
    ...
    "**Pattern 4：最小生成樹與並查集（DSU）**\n\n無向連通圖選 n-1 條邊讓總權最小。Kruskal = sort edges + DSU。DSU 本身也是獨立工具：動態加邊、合併集合、判斷加這條邊會不會成環，都是它的訊號。",
  ...
  greedy: [
    ...
    "**Pattern 2：反悔貪心**\n\n先接受選擇，若超出限制，就移除目前最差的一個。常用 heap。同一個 heap 也支撐「最小代價優先」這一類：Huffman 編碼、反覆合併兩堆的最小成本，都是每次取出當前最小的兩個再放回。",
  ...
  math: [
    ...
    "**Pattern 7：模運算與同餘類**\n\n訊號：問「和能被 k 整除」「差是 k 的倍數」「每 k 步回到同一狀態」。把元素按 `x % k` 分成同餘類，再用前綴餘數計數；C++ 的 `%` 對負數會給負值，取完要補一次 `+ k`。",
  ].join("\n\n"),
  ...
  monotonic_stack: [
    ...
    "**Pattern 4：字典序最小 subsequence**\n\nstack 維護答案，若當前字元更小且被彈字元後面還會出現，就可以彈出。「移除 k 位數使結果最小」是同一招的變形：改用剩餘可刪次數當彈出條件，刪不滿 k 個時再從尾端補刪。",
  ].join("\n\n"),
